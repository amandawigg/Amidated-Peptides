...
Each classically secreted protein is scanned N-terminus to C-terminus for a
glycine-dibasic tripeptide (GKR, GKK, GRK, GRR), indicating proprotein
convertase cleavage at the dibasic site followed by PAM-mediated amidation of
the resulting C-terminal glycine. An N-terminal cleavage site is then sought
upstream: the most 3' dibasic pair (KR, KK, RK, RR) downstream of the signal
peptide, otherwise the annotated signal peptide cleavage site. The predicted
peptide is the sequence between that site and the residue preceding the glycine.
...
Uncomment the species you want to run.
"""
...
output    = open('Mouse_amidated_peptides.csv', 'w', newline='')  ## Mouse

# secretome = open('human_secretome.tsv', 'r')                 ## Human
# output    = open('Human_amidated_peptides.csv', 'w', newline='')  ## Human
...
MAX_LENGTH       = 31   # longest peptide reported (exclusive)
FLANK            = 5    # residues of flanking sequence reported either side

# csv.writer quotes fields containing commas. UniProt annotation columns
# (peptide / propeptide / chain / modified residue) frequently contain them, so
# building rows by string concatenation shifts every later column on those rows.
...
for gene in secreted_genes:
    ...
    for position in range(len(seq) - 1):
        ...
        # The parentheses around the motif test are required. Without them
        # Python's precedence (and binds tighter than or) applies the position
        # test to 'GRK' only, admitting the other three motifs at positions 0-1.
        # Reported by a reviewer and corrected here; re-running produced no
        # change to the predicted set, as all candidates lay beyond position 1.
        if (seq[position:position + 3] in AMIDATION_MOTIFS) and (position > 1):
            ...
            # negative index silently wraps round to the C-terminus.
            if start > FLANK:
            ...
            if MIN_LENGTH <= len(pot_hormone) < MAX_LENGTH:
                if pot_hormone not in pep_list:
                    ...
                    # second site in a protein becomes PEP-GENE-2. Note the
                    # counter is per UniProt entry, so where one gene appears
                    # under several accessions the numbering must be reconciled
                    # downstream.
                    if count > 1:
